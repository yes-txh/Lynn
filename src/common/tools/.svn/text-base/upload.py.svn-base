...
DEFAULT_REVIEW_SERVER = "http://10.6.193.143:8080"
...
group.add_option('--files', action='store', dest='files',
                  help='only process specified files or dirs')
...
def GuessVCSName():
  ...
  if os.path.isdir('.svn') or os.path.isdir('_svn'):
...
def CodeReviewAuth(username, passwd):
  ...
  login_url = '%s/_ah/login' %(options.server)
...
def TryEncodeStr(instring, description):
  """ Try to use (assci, gbk, utf-8) encoding to decode data. other encodings
...
def FilterFilesData(data, files):
  """ Filter diff data according to options --skip and --files
      do encoding conversion
  """
  # filter files using --files
  if options.files:
    keep_list = []
    for keep in options.files.split():
      if os.path.isdir(keep):
        keep = os.path.normpath(keep) + os.path.sep
      keep_list.append(keep)
    keep_files = {}
    for file,info in files.items():
      for keep in keep_list:
        if file == keep or os.path.isdir(keep) and file.startswith(keep): 
  ...
  for line in data.splitlines(True):
    if line.startswith('Index:') or line.startswith('Property changes on:'):
      ...
      if filename in files:
        ...
        keep_line = False
    if keep_line:
      description = 'svn diff for %s' %filename
      line = TryEncodeStr(line, description)
  ...
  for filename, info in files.items():
    ...
    filename = TryEncodeStr(filename, 'filename')
    if not is_binary:
      if base_content:
        base_content = TryEncodeStr(base_content, 'base_content')
      if new_content:
        new_content = TryEncodeStr(new_content, 'new_content')
...
#  os.environ['LC_ALL'] = 'zh_CN.UTF-8'
  ...
  message = TryEncodeStr(message, 'message')
  ...
  if options.description_file:
    ...
    file.close()
  if description:
    description = TryEncodeStr(description, 'description')
