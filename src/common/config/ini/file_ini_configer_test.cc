...
// 设置脚手架
class FileConfigerTest : public testing::Test {
protected:
    virtual void SetUp() {
        // 分配配置文件对象
        m_ini_file  = new FileIniConfiger;
        m_file_name = "file_ini_configer_test.ini";
        m_field     = "ivan";

        ofstream file;
        file.open(m_file_name.c_str(), ios::out);
        assert(!file.fail());
...
protected:
    string       m_file_name;
    string       m_field;
    FileIniConfiger *m_ini_file;
};

// 测试整形
TEST_F(FileConfigerTest, Integer) {
    ASSERT_TRUE(m_ini_file->Load(m_file_name));
...
// 测试浮点型
TEST_F(FileConfigerTest, FloatingPoint) {
    ASSERT_TRUE(m_ini_file->Load(m_file_name));
