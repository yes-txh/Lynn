...
#include "common/base/uint32_divisor.hpp"
...
public:
    ...
    void Initialize(size_t element_count, double false_positive_prob)
    {
        ...
        {
            char message[128];
            sprintf(message, "Num elements=%lu false_positive_prob=%g",
        ...
        {
            char message[128];
            sprintf(message, "Bitmap too large, size=%llu, exceed size_t limitation", num_bytes);
            throw std::runtime_error(message);
        }
...
private:
    ...
    bool m_own_bitmap; ///< thether we own the bitmap
};
...
private:
    void Initialize(size_t element_count, double false_positive_prob)
    {
        ...
        if (num_bytes > ~size_t(0))
        {
            char message[128];
            sprintf(message,
                "Bitmap too large, size=%llu, exceed size_t limitation",
                num_bytes
            );
            throw std::runtime_error(message);
        }
