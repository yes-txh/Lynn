...
StreamImpl::~StreamImpl() {
    LOG(INFO) << "In stream destructor, id: " << m_stream_id;
    ClearQueue();
}
...
void ServerInputStream::ReadPacket(CompletionCallback* completion_callback) {
    ...
    {  // for scoped lock.
        ...
        if (m_buffered_request != NULL) {
            ...
            uint64_t timer_id =
                m_timer_manager->AddOneshotTimer(m_options.timeout(), closure);
...
void ClientOutputStream::UploadPacketComplete(const std::string* data,
        RpcController* controller,
