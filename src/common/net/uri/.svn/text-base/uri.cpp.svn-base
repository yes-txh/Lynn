...
void URI::EscapeString(const char* begin, const char* end, std::string* dest)
{
    dest->clear();
    const char* i = begin;
    while (i != end)
    {
        if (!Ascii::IsPrint(*i) || Ascii::IsSpace(*i))
        {
...
void URI::EscapeString(const char* begin, size_t length, std::string* dest)
{
    return URI::EscapeString(begin, begin + length, dest);
}

void URI::EscapeString(const std::string& src, std::string* dest)
{
    if (&src == dest)
    {
        std::string tmp;
        URI::EscapeString(src.data(), src.length(), &tmp);
        std::swap(tmp, *dest);
    }
    else
    {
        URI::EscapeString(src.data(), src.length(), dest);
    }
}

void URI::EscapeString(std::string* uri)
{
    std::string tmp;
    URI::EscapeString(uri->data(), uri->length(), &tmp);
    std::swap(*uri, tmp);
}
...
private: // in RFC 3986 Appendix A.  Collected ABNF for URI order
    ...
    {
        Result r(this);
        r = match_scheme() && match_literal(':') && match_hier_part();
        if (r)
        {
            {
                Result r1(this);
    ...
    bool match_scheme()
    {
        ...
        if (r)
        {
            ...
            m_result->SetScheme(r.begin(), r.length());
        }
        return r;
    }

    bool maybe_contains_userinfo()
    {
