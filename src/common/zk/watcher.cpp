...
// $Author: tyrionlin $
// $Revision: 41337 $
// $Description: collector zookeeper$
// $Id: watcher.cpp 41337 2011-05-19 08:53:55Z tyrionlin $
// $HeadURL$
=============================================================================
 */
#include "tborg/conf_manager/watcher.h"
#include "tborg/conf_manager/data_completion.h"
...
void Watcher(zhandle_t *zh, int type, int state,
    ...
    if (type == ZOO_SESSION_EVENT) {
        if (state == ZOO_CONNECTED_STATE) {
            ...
            LOG(INFO) << "Connect to server success, session id: "
                      << id->client_id;
        } else if (state == ZOO_AUTH_FAILED_STATE) {
            LOG(ERROR) << "Authentication failure. Shutting down...";
            zh = NULL;
        } else if (state == ZOO_EXPIRED_SESSION_STATE)   {
            LOG(ERROR) << "state == ZOO_EXPIRED_SESSION_STATE..........";
            zh = NULL;
        }
    }
#if 0
...
void ValueWatcher(zhandle_t *zh, int type, int state,
    ...
    if (type != ZOO_SESSION_EVENT) {
        ...
        if (NULL != zh) {
            ...
            if (rt != ZOK) {
                LOG(INFO) << "error happens when zoo_aget: " << zerror(rt);
            }
        }
    }
}
