...
int ZookeeperCommon::Init(const std::string& cluster_name, const std::string& hostports) {
    ...
    if (rt < 0) {
        LOG(ERROR) << "zk service will quit, can't find cluster info for this cluster: "
                   << cluster_name;
        return -1;
    }
...
int ZookeeperCommon::CheckPathExist(const std::string& path) {
    ...
    Stat stat;
    if (NULL == m_zk) {
        LOG(ERROR) << "m_zk is null";
        return -1;
    }
    int zrc = zoo_exists(m_zk, const_cast<char*>(path.c_str()), 0, &stat);
    if (zrc != ZOK) {
        LOG(WARNING) << "zoo_exists failed, the error is .. " << zerror(zrc);
        return -1;
    }
...
int ZookeeperCommon::WCheckPathExist(const std::string& path, watcher_fn watcher) {
    ...
    if (zrc != ZOK) {
        LOG(WARNING) << "zoo_get failed, the return value is " << zerror(zrc);
        return -1;
    }
    return 0;
}
...
int ZookeeperCommon::CreateNode(const std::string& node_path, bool is_temporary) {
    ...
    if (zrc != ZOK && zrc != ZNODEEXISTS) {
        LOG(WARNING) << "ERROR happens when create the node: " << node_path;
        LOG(WARNING) << "the error is .. " << zerror(zrc);
        return -1;
    }
...
int ZookeeperCommon::CreateNodeForce(const std::string& node_path) {
    LOG(INFO) << "begin to create a node in zookeeper forcelly.";
    LOG(INFO) << "path: " << node_path;
...
int ZookeeperCommon::CreateNodeWithValue(const std::string& node_path, const std::string& value, bool is_temporary) {
    ...
    LOG(INFO) << "the node with value is created successfully.";
    return 0;
}
...
int ZookeeperCommon::SetValueOfNode(const std::string& path, const std::string& value) {
    ...
    if (zrc != ZOK) {
        LOG(WARNING) << "data set failed... " << path.c_str() << " " << zerror(zrc);
        return -1;
    }
    return 0;
}
...
int ZookeeperCommon::GetValueOfNode(const std::string& path, std::string *value) {
    ...
    if (zrc != ZOK) {
        LOG(ERROR) << "zoo_get failed, the return value is " << zerror(zrc);
        return -1;
    }
    *value = buffer;
    return 0;
...
int ZookeeperCommon::WGetValueOfNode(const std::string& path,
    ...
    if (zrc != ZOK) {
        LOG(ERROR) << "zoo_wget failed, the return value is " << zerror(zrc);
        return -1;
    }
    *value = buffer;
    return 0;
...
int ZookeeperCommon::DeleteNode(const std::string& path) {
    ...
    if (rc != ZOK && rc != ZNONODE) {
        LOG(ERROR) << "zoo_delete failed, the return value is " << zerror(rc);
        return -1;
    }
    return 0;
}
...
int ZookeeperCommon::DeleteNodeForce(const std::string& path) {
    ...
    if (rc != ZOK) {
        LOG(WARNING) << "error happens when zoo_get_children: " << zerror(rc);
        return -1;
    }
    ...
    for (int i = 0; i < children.count; ++i) {
        ...
        if (ZOK == rt && grand_children.count > 0) {
            LOG(WARNING) << "The node " << path << "has grandchilden, it can't be deleted";
            return -1;
        }
    }
    ...
    for (int i = 0; i < children.count; ++i) {
        ...
        if (delete_rt != ZOK) {
            // LOG(WARNING) << "zoo_delete failed, the return value is " << zerror(delete_rt);
            continue;
        }
    }
    ...
    rc = zoo_delete(m_zk, const_cast<char*>(path.c_str()), -1);
    if (rc != ZOK && rc != ZNONODE) {
        // LOG(WARNING) << "zoo_delete failed, the return value is " << zerror(rc);
        return -1;
    }
    return 0;
}
...
int ZookeeperCommon::GetChildren(const std::string& path,
    ...
    for (int i = 0; i < children.count; ++i) {
        ...
        if (rc != ZOK) {
            LOG(WARNING) << "error happens when zoo_get: " << zerror(rc);
            continue;
        }
