...
#ifndef COMMON_BASELIB_SVRPUBLIB_BASE_PROTOCOL_T_INL_
#define COMMON_BASELIB_SVRPUBLIB_BASE_PROTOCOL_T_INL_
...
template<typename T>
bool    CBaseProtocolPack_<T>::CheckBuffer(uint32_t new_data_len) {
    bool b = false;
    if (m_use_internal_buff) {
        if (m_head.length + new_data_len > m_pack_buff_len) {
            ...
            if (buff) {
                ...
                b = false;
                LOG(ERROR) << "CBaseProtocolPack_::CheckBuffer(),"
                           " out of memory.";
            }
        } else
            b = true;
...
bool CBaseProtocolPack_<T>::SetReservedData(const unsigned char* reserved,
        const uint16_t len) {
    bool b = false;
    if (reserved && len && CheckBuffer(len)) {
...
void CBaseProtocolPack_<T>::ResetContent() {
    ...
    // auto seq number
    m_num_auto_increase_seq++;
    m_head.sequence = m_num_auto_increase_seq;
}
...
inline bool GetSeqNumInPack(const char* pack,
    ...
    bool b = false;
    if (seq &&
            pack && len >= sizeof(BasProtocolHead)) {
        const BasProtocolHead* ptr_head =
            reinterpret_cast<const BasProtocolHead*>(pack);
...
void CBaseProtocolUnpack_<T>::CheckTID(){
    if(m_last_thread_id == 0){
        m_last_thread_id = xfs::base::GetTID();        
    }else{
        int32_t tid = xfs::base::GetTID();
        if(tid != m_last_thread_id){
            LOG(ERROR) << "multi threads call same object, init in thread:" << m_last_thread_id
...
bool CBaseProtocolUnpack_<T>::Unpack() {
    ...
    if (m_reference_data && m_ref_data_len &&
        ...
        // 写binary log
        if (FLAGS_binary_log)
            WriteBinaryLog((const char*)m_reference_data, m_ref_data_len);
        ...
        LOG(ERROR) << "CBaseProtocolUnpack, Unpack(), "
    ...
    if (val && m_reference_data && m_seg_vals) {
        if (m_seg_vals[key].seq == m_seq &&    //  ? valid item
            ...
            if (m_seg_vals[key].seq == m_seq) {
                LOG(ERROR) << "Get key value fail, "
                           "data type mismatch."
                           "key = " << key << ", key-type:" << GetDataTypeStr(type)
                           <<", received data type:" << GetDataTypeStr(m_seg_vals[key].type)
    ...
    if (GetKeyVal(key, DTYPE_BYTES, &ptr, &len)) {
        if (len == sizeof(T_ValType)) {
            *val = reinterpret_cast<T_ValType*>(ptr);
            ...
            LOG(ERROR) << "get val fail, mismatch sizeof(data type)."
                       "sizeof(T_ValType):" << static_cast<int32_t>(sizeof(T_ValType)) << ", "
                       "received data type len:" << len;
        }
    }
    return b;
}
    ...
    if (GetKeyVal(key, DTYPE_BYTES, &ptr, &len)) {
        if ((len%sizeof(T_ValType)) == 0) {
            ...
            *array_items = int32_t(len/sizeof(T_ValType));
            ...
            LOG(ERROR) << "get val fail,mismatch sizeof(data type)."
                       "sizeof(T_ValType):" << static_cast<int32_t>(sizeof(T_ValType)) << ", "
                       "received data type len:" <<
                       static_cast<float>(len) /
                       static_cast<float>(static_cast<int32_t>(sizeof(T_ValType)));
        }
    }
    return b;
}
...
#endif // COMMON_BASELIB_SVRPUBLIB_BASE_PROTOCOL_T_INL_
