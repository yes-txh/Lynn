...
WorkThread::WorkThread(size_t max_fd_value):
    m_SocketContexts(max_fd_value + 1)
{
}
...
void WorkThread::ClearCommandEvent(const CommandEvent& event)
{
    ...
    case CommandEvent_AddSocket:
        assert(!event.Cookie.IsNull());
        event.Cookie.CastTo<SocketContext*>()->Close();
        delete event.Cookie.CastTo<SocketContext*>();
        break;
    default:
        break;
    }
