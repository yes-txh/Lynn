...
int StreamSocketContext::CheckAndIndicatePacket(char*& buffer, int& left_length)
{
    ...
    else if (m_DetectedPacketLength > static_cast<int>(m_MaxPacketLength))
    {
        // 数据包大小超出限制
...
int StreamSocketContext::SplitAndIndicatePackets()
{
    ...
    while (CheckReceivedPacketLength(begin, left_length, m_DetectedPacketLength))
    {
        int result = CheckAndIndicatePacket(begin, left_length);
...
bool StreamSocketContext::CheckReceivedPacketLength(const char* buffer,
        size_t size, int& packet_length) const
{
    packet_length = GetEventHandler()->DetectPacketSize(buffer, size);
