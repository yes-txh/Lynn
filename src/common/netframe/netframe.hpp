#ifndef COMMON_NETFRAME_NETFRAME_HPP
#define COMMON_NETFRAME_NETFRAME_HPP
...
public:
    ...
    /// @retval 0 成功，其他值失败
    int SendPacket(const StreamEndPoint& endpoint, Packet* packet,
                   bool urgent = false);
    ...
    /// @retval 0 成功
    int SendPacket(const DatagramEndPoint& endpoint, const SocketAddress& address, Packet* packet);
...
private:
    ...
    /// @bried 设置一个socket为非阻塞的
    /// @param fd 被设置为非阻塞的Socket 描述符
    static int SetNonBlock(int fd);

    /// @brief 设置一个Socket的接受缓冲区大小
    /// @param fd 被设置的描述符
    /// @param size Buffer的大小
    static int SetReceiveBufferSize(int fd, size_t size);

    /// @brief 设置一个Socket的发送缓冲区大小
    /// @param fd 被设置的描述符
    /// @param size Buffer的大小
    static int SetSendBufferSize(int fd, size_t size);

    /// @brief 设置一个Socket的Keep Alive属性
    /// @param fd 被设置的Socket 描述符
    static int SetKeepAlive(int fd);

    /// @brief 设置一个Socket的NoDelay属性
    /// @param fd 被设置的描述符
    static int SetTcpNoDelay(int fd);

    static int CloseFd(int fd);

    void IncreaseBufferedLength(size_t length);
    void DecreaseBufferedLength(size_t length);
    size_t GetCurrentBufferedLength() const;
    size_t GetMaxBufferedLength() const;

    void IncreaseBufferedPacket();
    void DecreaseBufferedPacket();
    size_t GetBufferedPacketNumber();
...
private:
    ...
    size_t m_MaxBufferedMemorySize;
    size_t m_CurrentBufferedMemorySize;
    size_t m_BufferedPacketNumber;
    std::vector<WorkThread*> m_WorkThreads;
};

} // namespace netframe

#endif // COMMON_NETFRAME_NETFRAME_HPP
