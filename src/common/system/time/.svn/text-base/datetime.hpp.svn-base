...
public:
    ...
    TimeSpan(int dwDay, int dwHour, int dwMinute, int dwSecond, int dwMilliSecond, int dwMicroSecond);
    ...
    static TimeSpan FromTick(TimeTick &tt);
    ///返回指定天的TimeSpan
...
class DateTimeFormatInfo
{
    ...
    //zh-CN,en-US等
    DateTimeFormatInfo(const std::string &cultureStr = "zh-CN");
public:
...
public:
    ...
    DateTime( int dwTimezoneSeconds = 28800);
    DateTime(const TimeTick &tmTick, int dwTimezoneSeconds = 28800);
    DateTime(const time_t &tm, int usec = 0,  int dwTimezoneSeconds = 28800);
    DateTime(int dwYear, int dwMonth , int dwDay ,
             int dwHour = 0, int dwMinute = 0, int dwSecond = 0, int dwMilliSecond = 0, int dwMicroSecond = 0, int dwTimezoneSeconds = 28800);
    ...
    std::string ToString(const std::string &strFormat, const DateTimeFormatInfo &formatInfo = DateTimeFormatInfo::zh_CNInfo)const;

    ///从时间串读取时间,按照指定的格式，创建失败会返回当前时间，默认为本地时间，缺失date用当前日期代替
    ///格式串详情参见MSDN文档 ms-help://MS.MSDNQTR.v90.chs/dv_fxfund/html/bb79761a-ca08-44ee-b142-b06b3e2fc22b.htm
    ///和ms-help://MS.MSDNQTR.v90.chs/dv_fxfund/html/98b374e3-0cc2-4c78-ab44-efb671d71984.htm
    static DateTime Parse(const std::string &strTime,const std::string &strFormat, const DateTimeFormatInfo &formatInfo = DateTimeFormatInfo::zh_CNInfo);

    ///从时间串读取时间,创建失败会返回当前时间,尽可能的把字符串转换为DateTime,缺失的年月日用当前年月日代替，没有time用12:00:00.000000代替,默认为本地时间
    ///该函数没有优化性能，如果可能请使用指定strFormat的函数版本
    static DateTime Parse(const std::string &strTime, const DateTimeFormatInfo &formatInfo = DateTimeFormatInfo::zh_CNInfo);

    ///从时间串读取时间,按照指定的格式，创建失败会返回当前时间，默认为本地时间，缺失date用当前日期代替
    ///格式串详情参见MSDN文档 ms-help://MS.MSDNQTR.v90.chs/dv_fxfund/html/bb79761a-ca08-44ee-b142-b06b3e2fc22b.htm
    ///和ms-help://MS.MSDNQTR.v90.chs/dv_fxfund/html/98b374e3-0cc2-4c78-ab44-efb671d71984.htm
    static bool TryParse(const std::string &strTime,const std::string &strFormat, DateTime & dt, const DateTimeFormatInfo &formatInfo = DateTimeFormatInfo::zh_CNInfo);

    ///从时间串读取时间,创建失败会返回当前时间,尽可能的把字符串转换为DateTime,缺失的年月日用当前年月日代替，没有time用12:00:00.000000代替,默认为本地时间
    static bool TryParse(const std::string &strTime, DateTime & dt, const DateTimeFormatInfo &formatInfo = DateTimeFormatInfo::zh_CNInfo);
...
private:
    ...
    ///展开预定义模式为自定义模式串
    std::string ExpandPredefinedFormat( const std::string &strFormat, const DateTimeFormatInfo &formatInfo, DateTime &dt ) const;
    ///根据格式串片段获取模式类别
    static bool ParseFormatSegment(FormatSegment &seg);
    ///格式串分割成一系列片段
    static bool SegmentFormatString(const std::string &strFormat, std::vector<FormatSegment> &vecSegment);
    ///将dt按照自定义模式串转化为字符串
    static std::string ToFormatString(const DateTime &dt, const std::string &strFormat,const DateTimeFormatInfo& formatInfo);
    ///将dt按照自定义模式串片段序列转化为字符串
    static std::string ToFormatString(const DateTime &dt, const std::vector<FormatSegment> &vecSegment,const DateTimeFormatInfo& formatInfo);
    ///匹配一个segment，返回成功与否
    static bool TryParseSegment( const std::string &strTime, size_t &dwIndex, const std::vector<FormatSegment> &vecSegment, const size_t dwSegIndex, DateTime &dt, const DateTimeFormatInfo &formatInfo );
};
