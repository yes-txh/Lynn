...
TimeSpan TimeSpan::FromTick(TimeTick& tt)
{
    return TimeSpan(tt);
}
...
DateTime::DateTime(const TimeTick& tmTick, int dwTimezoneSeconds /*= 28800*/)
{
    m_Time = tmTick;
    m_Time.Normalize();
    m_dwTimezoneSeconds = dwTimezoneSeconds;
}
...
DateTime::DateTime(int dwTimezoneSeconds /*= 28800*/)
{
    m_Time = DateTime::Now().m_Time;
    m_dwTimezoneSeconds = dwTimezoneSeconds;
}

DateTime::DateTime(const time_t& tm, int usec /*= 0*/, int dwTimezoneSeconds /*= 28800*/)
{
    m_Time.m_sec = tm;
    m_Time.m_usec = usec;
    m_Time.Normalize();
...
{
    return DateTime(this->m_Time - ts.m_Time);
}
...
bool DateTime::operator>(const DateTime& ts) const
{
    return this->m_Time > ts.m_Time;
}
...
int DateTime::GetYear() const
{
    struct tm   tmNow;
#ifdef _WIN32
    localtime_s(&tmNow, &this->m_Time.m_sec);
#else
    localtime_r(&this->m_Time.m_sec, &tmNow);
#endif
...
DateTime DateTime::Now()
{
    ...
    dwTimezoneSeconds = -tz.tz_minuteswest * 60;
...
DateTime DateTime::Today()
{
    ...
    _ftime_s(&tbNow);
    dwTimezoneSeconds = -tbNow.timezone * 60;
    TimeTick tt(tbNow.time - tbNow.time % SECONDS_PER_DAY + tbNow.timezone * 60, 0);
#else
    struct timeval  tvNow;
    struct timezone tz;
    gettimeofday(&tvNow, &tz);
    TimeTick tt(tvNow.tv_sec - tvNow.tv_sec % SECONDS_PER_DAY + tz.tz_minuteswest * 60, 0);
    dwTimezoneSeconds = -tz.tz_minuteswest * 60;
#endif
    return DateTime(tt, dwTimezoneSeconds);
}
...
TimeSpan DateTime::Time()
{
    ...
    struct _timeb   tbNow;
    _ftime_s(&tbNow);
    TimeTick tt(tbNow.time % SECONDS_PER_DAY - tbNow.timezone * 60, tbNow.millitm * 1000);
#else
    tzset();
    struct timeval  tvNow;
    gettimeofday(&tvNow, NULL);
    TimeTick tt(tvNow.tv_sec % SECONDS_PER_DAY - timezone, (int) tvNow.tv_usec);
#endif
    return TimeSpan(tt);
}

DateTime DateTime::Parse(const std::string& strTime, const DateTimeFormatInfo& formatInfo /*= DateTimeFormatInfo::zh_CNInfo*/)
{
    DateTime dt;
    ...
    const DateTimeFormatInfo& formatInfo /*= DateTimeFormatInfo::zh_CNInfo*/)
{
    ...
    {
        if (!TryParseSegment(strTime, dwIndex, vecSegment, dwSegIndex, dt, formatInfo))
            return false;
    }
    ...
    const DateTimeFormatInfo& formatInfo /*= DateTimeFormatInfo::zh_CNInfo*/)
{
    ...
    for (vector<string>::const_iterator itr = vecAllPattern.begin(); itr != vecAllPattern.end(); ++itr)
    {
        dt = tm;
    ...
    std::vector<FormatSegment> &vecSegment)
{
    for (size_t dwIndex = 0; dwIndex < strFormat.length(); dwIndex++)
    {
        ...
        else if (IS_PATTERN_CHAR(ch))
        {
            ...
            while (strFormat[dwPos] == ch && dwPos < strFormat.length())
                dwPos++;
...
void DateTime::SetHour(int dwHour)
{
    m_Time.m_sec = (m_Time.m_sec - m_Time.m_sec % SECONDS_PER_DAY +
                        m_Time.m_sec % SECONDS_PER_HOUR + m_Time.m_sec % 60) +
    ...
    case FSTK_K:
        {
            ...
            // K需要设置时区
            dt.SetTimeZoneSeconds(dwTimezoneSeconds);
        }
        return false;
        // m     0-59
    case FSTK_m:
    ...
    case FSTK_zzz:
        {
            ...
            // zzz需要验证本地时区
            if (DateTime::Now().GetTimeZoneSeconds() != dwTimezoneSeconds)
            {
                return false;
            }
