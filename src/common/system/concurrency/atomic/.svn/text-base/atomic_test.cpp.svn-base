...
struct type_tester_fp
{
    ...
    {
        Atomic<T> n = 1.0f;
        ASSERT_DOUBLE_EQ(n, 1);

        T t;

        t = (n = 2);
        ASSERT_DOUBLE_EQ(n, 2);
        ...
        ASSERT_DOUBLE_EQ(n, 4);

        n = 4;
        ASSERT_TRUE(n.CompareExchange(4, 5, t));
        ASSERT_DOUBLE_EQ(t, 4);
        ASSERT_DOUBLE_EQ(n, 5);

        n = 5;
        ASSERT_FALSE(n.CompareExchange(4, 6, t));
        ASSERT_DOUBLE_EQ(t, 5);
        ASSERT_DOUBLE_EQ(n, 5);
    }
};
...
#define TEST_TYPE(type) do { type_tester<type>::run(#type); /*UNIT_TEST_INFO("test %s pass", #type);*/ } while (0)
...
class RWLocked
{
    static const size_t PointerBits = sizeof(void*) == 8 ? 48 : 32;
private:
    union Internal
    {
        struct
...
public:
    ...
    T* Acquire()
    {
        ...
        while (!AtomicCompareExchange(m_internal.as_uint64, to_compare.as_uint64, to_exchange.as_uint64, to_compare.as_uint64));

        return (T*)to_compare.ptr;
    }
...
TEST(Atomic, Pointer)
{
    ...
    int *pp = p;
    pp = p.Value();
    ASSERT_EQ((uintptr_t)p.Value(), 0U);
    int n;
    pp = p.Exchange(&n);
    ASSERT_EQ((uintptr_t)p.Value(), (uintptr_t)&n);
    ASSERT_EQ((uintptr_t)pp, 0U);
}
